 ...
 * 機能:
 * - JWT認証の有効/無効制御
 * - デバッグモード時のデフォルトユーザー処理
 * - 認証情報からユーザーIDを取得する共通メソッド
 * - 全Controllerで使用する共通処理を提供
 *
 * 使用方法:
 * - 各Controllerクラスでこのクラスを継承
 * - getUserIdFromAuth()メソッドで認証情報からユーザーIDを取得
 * - デバッグモード時は自動的にデフォルトユーザー（user1）を使用
 *
 * デバッグモード:
 * - jwt.auth.enabled=false の場合、認証をスキップ
 * - 全APIリクエストがデフォルトユーザー（user1）として実行
 * - トークンなしでAPIテストが可能
 */
@Slf4j
public abstract class BaseController {
    ...
    /**
     * 認証情報からユーザーIDを取得
     * デバッグモード時はデフォルトユーザーを使用
     *
     * 処理フロー:
     * 1. 通常モード：JWT認証からusernameを取得
     * 2. デバッグモード：デフォルトユーザー名を使用
     * 3. usernameでUserエンティティを検索
     * 4. 見つかったUserのIDを返却
     *
