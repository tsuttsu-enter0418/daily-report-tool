 ...
 * 日報データアクセス層
 *
 * <p>機能: - 日報の基本CRUD操作 - ユーザー別日報検索 - ステータス別フィルタリング - 日付範囲での検索 - 上司による部下日報検索
 *
 * <p>主要メソッド: - findByUserId: 特定ユーザーの日報一覧取得 - findByUserIdAndReportDate: 特定ユーザーの特定日日報取得 -
 * findByUserIdAndStatus: ステータス別日報検索 - findBySupervisorId: 上司による部下日報一覧取得 - findByReportDateBetween:
 * 日付範囲検索
 */
@Repository
public interface DailyReportRepository extends JpaRepository<DailyReport, Long> {
    ...
    /**
     * 上司の部下全員の日報一覧を取得 部下のユーザーIDを指定して、その部下たちの日報を取得
     *
     * @param subordinateUserIds 部下のユーザーIDリスト
     * @return 部下の日報一覧
     */
    @Query(
            "SELECT dr FROM DailyReport dr WHERE dr.userId IN :subordinateUserIds ORDER BY"
                    + " dr.reportDate DESC, dr.userId")
    List<DailyReport> findByUserIdInOrderByReportDateDescUserIdAsc(
     ...
     */
    @Query(
            "SELECT dr FROM DailyReport dr WHERE dr.userId IN :subordinateUserIds AND dr.status ="
                    + " :status ORDER BY dr.reportDate DESC, dr.userId")
     ...
     * @return 日報一覧
     */
    List<DailyReport> findByReportDateBetweenOrderByReportDateDescUserIdAsc(
