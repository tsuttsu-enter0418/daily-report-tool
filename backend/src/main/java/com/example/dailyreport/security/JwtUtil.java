 ...
 * 
 * 機能:
 * - JWTトークンの生成（ユーザー名、役職を含む）
 * - トークンからのユーザー情報抽出
 * - トークンの有効性検証
 * - トークンの期限切れチェック
 * 
 * セキュリティ:
 * - HMAC SHA-256署名アルゴリズム使用
 * - 設定可能な秘密鍵とトークン有効期限
 * - トークン改ざん検知機能
 * 
 * 設定値:
 * - jwt.secret: JWT署名用秘密鍵（デフォルト: mySecretKey）
 * - jwt.expiration: トークン有効期限（ミリ秒、デフォルト: 86400000 = 24時間）
 */
@Component
public class JwtUtil {
    ...
    /**
     * JWT署名用の秘密鍵を生成
     * HMAC SHA-256アルゴリズムに適したSecretKeyを返す
     * 
    ...
    /**
     * JWTトークンを生成
     * ユーザー名をsubject、役職をclaimとして含むトークンを作成
     * 
