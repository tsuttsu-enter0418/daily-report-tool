...
const mockDailyReports: DailyReportResponse[] = [
  {
    ...
    workContent: "本日は日報管理システムのフロントエンド改修を実施しました。具体的には、DailyReportListコンポーネントのパフォーマンス最適化を行い、メモ化を活用して不要な再レンダリングを防ぎました。また、ChakraUI v3.2の最新API仕様に対応するため、Card.RootやField.Rootの複合コンポーネント構造に移行しました。テストコードも併せて更新し、全11テストケースが正常に動作することを確認しています。明日は削除機能の確認ダイアログ実装を予定しています。",
    status: "submitted",
  ...
  },
  {
    ...
    workContent: "Spring Boot側のデイリーレポートAPIとの統合テストを実施しました。JWT認証フローが正常に動作し、CORS設定も問題ないことを確認。PostgreSQLとの接続も安定しています。",
    status: "draft",
  ...
  },
  {
    ...
    workContent: "午前中はデータベース設計のレビュー会議に参加しました。users テーブルとdaily_reportsテーブルの関係性、インデックス設計について議論。supervisor_id による階層構造の実装方針を確認しました。午後はpgAdminを使用してテストデータの投入を行い、N+1問題が発生しないようクエリ最適化を実施。パフォーマンステストの結果、想定通りの応答時間を達成できています。",
    status: "submitted",
  ...
  },
  {
    ...
    workContent: "テストの安定性向上のため、Vitestの設定を見直しました。非同期処理のタイムアウト問題を解決し、ChakraProviderの統合テストエラーも修正。テストユーティリティを作成して再利用可能にしています。",
    status: "draft",
  ...
  },
  {
    ...
    workContent: "今週は開発環境の改善に注力しました。GitHub Actions を使用したCI/CDパイプラインを構築し、プルリクエスト時に自動でビルド・テストが実行される仕組みを整備。Docker Compose設定も見直し、frontend、backend、PostgreSQL、pgAdminの統合環境を構築しました。multi-stage buildによるイメージサイズ最適化も実施し、本番デプロイ時間を30%短縮。開発者の生産性向上とデプロイリスク軽減を両立できる環境が完成しました。来週は監視ツールの導入を検討予定です。",
    status: "submitted",
  ...
  },
  {
    ...
    workContent: "プロジェクト全体の型安全性を向上させました。interface から type エイリアスへの移行を完了し、ユニオン型と型合成を活用。ESLintエラーを97%削減（232→3件）し、本番環境でのランタイムエラーを大幅に減らしました。",
    status: "submitted",
  ...
  },
  {
    ...
    workContent: "WebアプリケーションのアクセシビリティをWCAG 2.1 AA準拠レベルまで向上させる作業を実施。Spinnerコンポーネントにaria-label属性を追加し、スクリーンリーダー対応を強化。キーボードナビゲーションの改善とカラーコントラスト比の調整も完了。障害者支援技術での動作確認を行い、より多くのユーザーが利用できるインクルーシブな設計を実現しました。",
    status: "submitted",
  ...
  },
  {
    ...
    displayName: "田中太郎",
    title: "今日のタスク整理中...",
    workContent: "今日は朝のスタンドアップで確認したタスクをまとめています。まだ作業途中なので後で詳細を記載予定です。",
    status: "draft",
...
export const mockApi = {
   ...
   * @returns 現在ユーザーの日報一覧
   */
  async getDailyReports(token: string, params?: DailyReportListParams): Promise<DailyReportResponse[]> {
    await delay(600);
   ...
   * @returns 作成された日報
   */
  async createDailyReport(token: string, reportData: DailyReportCreateRequest): Promise<DailyReportResponse> {
    await delay(800);
    ...
    const existingReport = mockDailyReports.find(
      report => report.userId.toString() === user.id && report.reportDate === reportData.reportDate
    );
  ...
  async getDailyReport(token: string, id: number): Promise<DailyReportResponse | null> {
    ...
    const report = mockDailyReports.find(
  ...
  async updateDailyReport(
    ...
    if (reportData.reportDate !== mockDailyReports[reportIndex].reportDate) {
      const existingReport = mockDailyReports.find(
        report => report.userId.toString() === user.id && 
    ...
    mockDailyReports[reportIndex] = {
      ...
      status: reportData.status,
      reportDate: reportData.reportDate,
      submittedAt: reportData.status === "submitted" && previousStatus === "draft" ? now : mockDailyReports[reportIndex].submittedAt,
      updatedAt: now
    };
