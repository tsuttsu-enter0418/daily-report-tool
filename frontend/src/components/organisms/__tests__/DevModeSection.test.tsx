...
describe("DevModeSection", () => {
  it("開発モードでない場合は何も表示されない", () => {
    const { queryByText } = render(<DevModeSection isDevelopment={false} useRealAPI={false} />);
  ...
  it("本番モードでは何も表示されない", () => {
    const { queryByText } = render(<DevModeSection isDevelopment={false} useRealAPI={true} />);
